resource "volterra_origin_pool" "sentence-animals-pool" {
  name                   = "tf-sentence-animals-pool"
  ...
  origin_servers {
    ...
    k8s_service {
      ...
      service_name   = "sentence-animals.r-bruenig"
...
resource "volterra_http_loadbalancer" "sentence-animals-lb" {
  name      = "tf-sentence-animals-lb"
  namespace = var.namespace

  domains = ["sentence-animals.r-bruenig"]
  http {
...
#    }
  ...
  default_route_pools {
    pool {
      name = volterra_origin_pool.sentence-animals-pool.name
      namespace = var.namespace
    }
    weight = 1
  }
